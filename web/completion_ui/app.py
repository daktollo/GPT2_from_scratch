...
def load_gpt_model():
    ...
    print("Loading GPT-2 model for completion...")
    ...
    print(f"Completion model loaded successfully on {device}")
...
def complete():
    ...
    try:
        ...
        # Generate completion
        idx = text_to_token_ids(user_text, tokenizer)
        idx = idx.to(device)
        ...
        # Get the full text and extract only the completion part
        full_text = token_ids_to_text(token_ids, tokenizer)
        completion = full_text[len(user_text):].strip()
